...
class UserDataSource {
    ...
    func loadLocalUsers() {
        guard let data = self.realm?.objects(StoredUser.self).sorted(byKeyPath: "insertDate", ascending: true) else {
            return
        }
        
        let items: [User] = data.map({
            let model = User(userFullName: UserFullName(title: "", firstName: $0.firstName ?? "", lastName: $0.lastName ?? ""), phoneNumber: $0.phoneNumber ?? "", userPhotoURL: UserAvatar(large: $0.avatarURLs?.large ?? "", thumb: $0.avatarURLs?.thumbnail ?? ""), userEmail: $0.email ?? "")
    ...
    func saveUser(_ user: User) {
        ...
        if let storedObject = object {
            ...
            storedObject.avatarURLs?.thumbnail = localUser.avatarURLs?.thumbnail
            storedObject.avatarURLs?.large = localUser.avatarURLs?.large
